...
<#
    .SYNOPSIS
        Central Crossplatform PowerShell Profile
    .DESCRIPTION
        This PS Profile can be used into your local PS Profile on Mac, Linux, or Windows 
        ...
        This profile can be loaded directly from a Secret Gist as well - Merging this PS Profile into your local PS Profile:
    ...
    .NOTES
        PS on every Platform!
#>
# This profile = local Microsoft.Powershell_profile.ps1 @ $Profile.CurrentUserAllHosts

# Set Path
if($IsMacOS -eq $true) {
  $default_home = "$home/.config/PowerShell"
...
# Set IsUnix=True if we're on a Unix-based OS running POwershell 
  If (($IsACS -eq $true) -or ($IsMacOS -eq $true) -or ($IsLinux -eq $true))
  {
    $IsUnix = $true
  } else {
    $IsUnix = $false
...
# If we're running on Powershell 5 let's set the $IsWindows variable.  No need to do this on PS Version 6+
  If (($PSVersionTable.PSVersion.ToString() -lt 6) -and ($env:OS -eq "Windows_NT")) {
    $IsWindows=$true
  }

#If we're running in Azure Cloud Shell, spit out the current Tenant info 
   If ($IsACS -eq $true) {
     Write-Verbose -Verbose -Message "Active Azure Subscription = $((Get-AzContext).Name)"
   }
...
# don't override chocolatey sudo or unix sudo
  ...
  function Edit-PSProfile {
      If($IsACS -eq $true){  #only do this if we're *not* in an Azure Cloud Shell session
          write-host $profile
          code /home/username/.config/PowerShell/Microsoft.PowerShell_profile.ps1
      }
...
# already expanded to save time https://github.com/nvbn/thefuck/wiki/Shell-aliases#powershell
  ...
  Function ManageTenant {
          ...
          If($IsACS -eq $true){
              ...
              If  ($IsWindows -eq $true) {Connect-SPOService -url https://yourtenant-admin.sharepoint.com}
...
} # interactive test close#Requires -Version 5
using namespace System.Management.Automation
using namespace System.Management.Automation.Language
<#
    .SYNOPSIS
