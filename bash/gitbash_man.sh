...
if [ -z "$query" ]; then
  printf 'Provide the man page you want to see. E.g. > man [manpage]\n' >&2
  exit 1
fi
...
mapfile -t selected < <(fd --color=never \
  --follow \
  --type=file \
  --exclude '*.mk' \
...
if [ -z "$selected" ]; then
  ...
  mapfile -t selected < <(printf '%s\n' "${selected[@]}" | fzf --prompt='Man> ' --no-multi \
    --ansi --cycle \
    --input-border \
...
# Build and display
if get_man_content | mandoc -man 2>/dev/null | col -bx > "$tempFile"; then
  bat --color=always --style=plain --language man "$tempFile"
else
  get_man_content | bat --color=always --style=plain
fi
